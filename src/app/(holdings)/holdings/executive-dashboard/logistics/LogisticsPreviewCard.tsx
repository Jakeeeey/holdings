...
export function LogisticsPreviewCard({ group }: { group: { id: number; group_name: string; [key: string]: unknown } }) {
    ...
    useEffect(() => {
        const load = async () => {
            try {
                ...
                setFulfillmentRate(computeFulfillmentRate(rows as any));
            } catch (err) {
    ...
    return (
        <Link href={`/holdings/logistics-performance/driver-kpi`} className="block h-full cursor-pointer group">
            <Card className="relative overflow-hidden border-border/20 bg-[#0a0a0b] hover:border-emerald-500/30 hover:shadow-[0_0_30px_rgba(16,185,129,0.1)] transition-all duration-500 flex flex-col h-full min-h-[260px]">
                ...
                <CardHeader className="border-b border-white/[0.05] bg-white/[0.02] pb-4 relative z-10 backdrop-blur-sm">
                    <div className="flex items-center justify-between">
                        <div className="flex flex-col gap-1">
                            <CardTitle className="text-xl font-black uppercase tracking-tight italic flex items-center gap-2 drop-shadow-md">
                        ...
                        <div className="p-2.5 bg-black/40 rounded-xl border border-white/[0.05] shadow-inner group-hover:bg-emerald-500/10 group-hover:border-emerald-500/30 transition-all duration-500">
                ...
                <CardContent className="flex-1 p-5 sm:p-6 flex flex-col relative z-10 justify-center gap-4">
                    {loading ? (
                        <div className="flex flex-col gap-4 w-full">
                            <Skeleton className="h-20 w-full rounded-xl bg-white/[0.02]" />
                            <div className="grid grid-cols-2 gap-3">
                                <Skeleton className="h-20 w-full rounded-xl bg-white/[0.02]" />
                                <div className="flex flex-col gap-2 justify-between">
                                    <Skeleton className="h-[36px] w-full rounded-lg bg-white/[0.02]" />
                                    <Skeleton className="h-[36px] w-full rounded-lg bg-white/[0.02]" />
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col gap-4">
                            ...
                            <div className="bg-emerald-500/5 border border-emerald-500/20 rounded-xl p-4 sm:p-5 relative overflow-hidden group/rate hover:bg-emerald-500/10 transition-colors duration-300 shadow-inner">
                                ...
                                <div className="flex justify-between items-end mb-3 relative z-10">
                                    <div className="flex flex-col gap-1.5">
                                        <div className="flex items-center gap-2 text-emerald-500 drop-shadow-sm">
                                            <CheckCircle className="w-4 h-4" />
                                            <span className="text-[10px] font-black uppercase tracking-[0.2em]">Fulfillment Rate</span>
                                        </div>
                                        <div className="flex items-baseline gap-1">
                                            <span className="text-4xl font-black italic text-emerald-400 leading-none drop-shadow-md">
                                                {fulfillmentRate}%
                                            </span>
                                        </div>
                                    </div>
                                    <Badge className={`px-2 py-1 text-[9px] uppercase tracking-widest border shadow-sm ${isExcellent ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30' : 'bg-amber-500/20 text-amber-400 border-amber-500/30'}`}>
                                        {isExcellent ? "Excellent" : "Needs Attention"}
                                    </Badge>
                                </div>

                                <div className="w-full bg-black/60 h-1.5 rounded-full overflow-hidden relative z-10 shadow-inner border border-white/[0.05]">
                                    <div 
                                        className={`h-full transition-all duration-1000 ease-out ${isExcellent ? 'bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.8)]' : 'bg-amber-500 shadow-[0_0_10px_rgba(245,158,11,0.8)]'}`} 
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                {/* Revenue / Value */}
                                <div className="bg-white/[0.03] border border-white/[0.05] rounded-xl p-4 flex flex-col justify-center gap-2.5 hover:bg-white/[0.06] transition-colors duration-300 group/item shadow-sm">
                                    ...
                                    <span className="text-lg sm:text-xl font-black italic text-foreground tracking-tight truncate drop-shadow-sm">
                                ...
                                {/* Timeliness Stats */}
                                <div className="flex flex-col gap-2 justify-between">
                                    <div className="bg-white/[0.03] border border-white/[0.05] rounded-lg p-2.5 px-3 flex justify-between items-center hover:bg-white/[0.06] transition-colors duration-300 shadow-sm">
                                        <div className="flex items-center gap-2 text-muted-foreground">
                                            <Clock className="w-3.5 h-3.5 text-amber-500 drop-shadow-sm" />
                                    ...
                                    <div className="bg-white/[0.03] border border-white/[0.05] rounded-lg p-2.5 px-3 flex justify-between items-center hover:bg-white/[0.06] transition-colors duration-300 shadow-sm">
                                        <div className="flex items-center gap-2 text-muted-foreground">
                                            <Clock className="w-3.5 h-3.5 text-indigo-500 drop-shadow-sm" />
