...
import { ProductSalesPreviewCard } from "../ProductSalesPreviewCard";
...
async function getProductSalesGroups(): Promise<GroupItem[]> {
    try {
        ...
        const res = await fetch(`${baseUrl}/api/holdings/dashboard-api-groups?category=sales-by-product`, { cache: "no-store" });
        if (res.ok) {
            const groups: GroupItem[] = await res.json();
            return groups.filter((g) => g.category === "sales-by-product");
        }
    } catch (e) {
        console.error("Failed to fetch product sales groups:", e);
    }
    return [];
}

export default async function SalesByProductGroupListPage() {
    const groups = await getProductSalesGroups();
            ...
            <main className="min-h-0 min-w-0 flex-1 overflow-y-auto p-6 md:p-10 text-foreground">
                <div className="max-w-7xl mx-auto space-y-10">
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 pb-2">
                        <div className="space-y-2">
                            <div className="flex items-center gap-2 text-muted-foreground/60 mb-1 text-[10px] uppercase font-black tracking-[0.2em]">
                                ...
                                <span className="text-primary font-black">Sales by Product</span>
                            </div>
                            <h2 className="text-4xl font-black tracking-tight text-foreground uppercase italic leading-none">
                                PRODUCT <span className="text-primary">REVENUE MIX</span>
                            </h2>
                            <p className="text-xs text-muted-foreground font-medium uppercase tracking-widest">
                                Top 5 Product line share &amp; gross revenue contribution across business units
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
                        {groups.map((group) => (
                            <ProductSalesPreviewCard key={group.id} group={group} />
                        ))}
                    </div>
                    ...
                    {groups.length === 0 && (
                        <div className="py-16 text-center text-muted-foreground border border-dashed rounded-2xl border-border/60">
                            ...
                            <p className="font-semibold uppercase tracking-wider text-sm">No Sales Groups Found</p>
