...
async function fetchDirectus<T>(path: string, groupId: string, req: NextRequest): Promise<T> {
  const url = `${getBaseUrl(req)}/api/holdings/dashboard/directus/${groupId}${path}`;
  ...
  if (!res.ok) {
    ...
    throw new Error(`Directus proxy error (${res.status}): ${text}`);
  }
...
export async function GET(req: NextRequest) {
  try {
    ...
    const tsdRes = await fetchDirectus<{ data: Record<string, unknown>[] }>(tsdUrl, groupId, req);
    ...
    const tssRes = await fetchDirectus<{ data: Record<string, unknown>[] }>(tssUrl, groupId, req);
    ...
    if (tssIds.length > 0) {
      ...
      const tspRes = await fetchDirectus<{ data: Record<string, unknown>[] }>(tspUrl, groupId, req);
      ...
      if (tspIds.length > 0) {
        ...
        const tslRes = await fetchDirectus<{ data: Record<string, unknown>[] }>(tslUrl, groupId, req);
