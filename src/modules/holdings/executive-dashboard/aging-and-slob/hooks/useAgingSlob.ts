...
export interface UseAgingSlobOptions {
  groupId?: string | number;
}

export function useAgingSlob(options?: UseAgingSlobOptions) {
  ...
  const fetchData = useCallback(async () => {
    try {
      setIsLoading(true);
      const result = await fetchAgingSlobData(groupId);
      setData(result);
      setError(null);
    } catch (err) {
    ...
    return data
      .filter((item) => {
        ...
        const supplierMatch =
          ...
          item.supplierShortcut === selectedSupplier ||
          item.branchName === selectedSupplier;

        // Branch Filter
        const branchMatch =
          selectedBranch === "all" || item.branchName === selectedBranch;
        ...
        if (dateRange?.from && item.lastOutboundDate) {
          try {
            const outboundDate = parse(
              item.lastOutboundDate,
              "yyyy-MM-dd HH:mm:ss",
              new Date(),
            );
