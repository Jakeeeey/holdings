...
function ManagerialSupplierContent({ groupId }: { groupId: string }) {
    ...
    const [selectedDivisionId, setSelectedDivisionId] = useState<number | null>(searchParams.get("divisionId") ? Number(searchParams.get("divisionId")) : null);
    ...
    useEffect(() => {
        ...
        const divIdParam = searchParams.get("divisionId");

        const timer = setTimeout(() => {
            if (divParam) setSelectedDivision(divParam.toUpperCase());
            if (divIdParam) setSelectedDivisionId(Number(divIdParam));
    ...
    useEffect(() => {
        const loadSalesmen = async () => {
            try {
                const res = await fetch("/api/holdings/executive-dashboard/metadata/salesmen");
                if (res.ok) {
    ...
    const salesmanDetailData = useMemo(() => {
        ...
        return rawData.filter(d => {
            ...
            return (
                ...
                (selectedDivisionId ? d.divisionId === selectedDivisionId : (d.divisionName || "").toUpperCase() === (selectedDivision || "").toUpperCase())
            );
        });
    }, [rawData, selectedSalesmanForModal, selectedSupplier, selectedDivision, selectedDivisionId, salesmenMap]);
    ...
    useEffect(() => {
        const load = async () => {
            try {
                ...
                const data = await fetchManagerialData(start, end, groupId);
                setRawData(data);

                // Scope targets to selected division
                const divItem = data.find(d => selectedDivisionId ? d.divisionId === selectedDivisionId : (d.divisionName || "").toUpperCase() === (selectedDivision || "").toUpperCase());
                const targetData = await fetchDynamicTargets(start, end, groupId, divItem?.divisionId || selectedDivisionId || undefined);
                setTargets({
        ...
        load();
    }, [fromMonth, toMonth, selectedDivision, selectedDivisionId, groupId]);
    ...
    const { supplierPerformance, divisionSummary } = useMemo(() => {
        ...
        // Filter by Division
        const filtered = rawData.filter(d => selectedDivisionId ? d.divisionId === selectedDivisionId : (d.divisionName || "").toUpperCase() === (selectedDivision || "").toUpperCase());
    ...
    }, [rawData, selectedDivision, selectedDivisionId, fromMonth, toMonth, targets]);
    ...
    const salesmanBreakdown = useMemo(() => {
        ...
        return Array.from(salesmanMap.entries())
            ...
            .sort((a, b) => b.sales - a.sales);
    }, [rawData, selectedSupplier, targets, selectedDivision, selectedDivisionId, fromMonth, toMonth, salesmenMap]);
                    ...
                    <div className="flex items-center gap-2 px-4 py-1">
                        <Filter className="h-4 w-4 text-primary" />
                            <Select value={selectedDivision} onValueChange={(val) => {
                                ...
                                setSelectedDivisionId(null); // Clear ID since user manually changed division name
...
export default function ManagerialSupplierModule({ groupId }: { groupId: string }) {
    return (
        <Suspense fallback={<div className="flex h-screen w-full items-center justify-center bg-background"><Loader2 className="h-8 w-8 animate-spin text-muted-foreground" /></div>}>
            <ManagerialSupplierContent groupId={groupId} />
        </Suspense>
    );
}