 ...
 */
export const fetchDynamicTargets = async (startDate: string, endDate: string, groupId: string, divisionId?: number): Promise<Record<string, unknown>> => {
    try {
        const params = new URLSearchParams({ startDate, endDate, groupId });
        if (divisionId) params.append("divisionId", String(divisionId));
        const url = `/api/holdings/executive-dashboard/managerial-supplier/targets?${params.toString()}`;
...
export const fetchCustomerPeaks = async (names: string[] = [], salesmanIds: number[], viewType: 'customer' | 'area' | 'storeType' = 'customer', storeType?: string): Promise<Record<string, { total: number; peak: number; metadata?: Record<string, unknown> }>> => {
    let url = `/api/holdings/executive-dashboard/managerial-supplier/customer-peak?ids=${salesmanIds.join(",")}${names.length > 0 ? `&names=${encodeURIComponent(names.join("|"))}` : ""}&viewType=${viewType}`;
...
export const fetchCustomerTargets = async (salesmanIds: number[], startDate: string, endDate: string, viewType: 'customer' | 'area' | 'storeType' = 'customer', names: string[] = []): Promise<Record<string, number>> => {
    try {
        const url = `/api/holdings/executive-dashboard/managerial-supplier/customer-targets?salesmanIds=${salesmanIds.join(",")}&startDate=${startDate}&endDate=${endDate}&viewType=${viewType}${names.length > 0 ? `&names=${encodeURIComponent(names.join("|"))}` : ""}`;
        const res = await fetch(url, { cache: "no-store" });
        if (!res.ok) return {};
...
export const fetchCustomerProducts = async (
    ...
    const url = `/api/holdings/executive-dashboard/managerial-supplier/customer-products?${params.toString()}`;
    const res = await fetch(url, { cache: "no-store" });
    if (!res.ok) return [];
    return await res.json();
};
