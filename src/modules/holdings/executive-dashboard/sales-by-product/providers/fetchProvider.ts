...
export async function fetchProductSalesData(
  ...
  const groupId = options?.groupId != null ? String(options.groupId) : "";
  ...
  const url = groupId
    ? `/api/holdings/dashboard/sales/${groupId}/api/view-sales-product-performance/all?${params.toString()}`
    : `/api/bia/crm/sales-report/product-sales-performance?${params.toString()}`;
